...
def spatialSplits(primitives, *args, **kwargs):
    ...
    clusters = [BinaryCluster(p) for p in morton_sort(primitives)]
    return [_spatial_splits(clusters, *args, **kwargs)]


def _spatial_splits(clusters, metric=Cluster.SAH_EPOMetric, partition=linear_split, **kwargs):
    if len(clusters) == 1:
        return clusters[0]
    l, r = partition(clusters, metric)
    if l and r:
        lcluster, rcluster = _spatial_splits(
            l, metric, partition), _spatial_splits(r, metric, partition)
        return BinaryCluster((lcluster, rcluster))
    elif l:
        lcluster = _spatial_splits(l, metric, partition)
        return lcluster
    elif r:
        rcluster = _spatial_splits(r, metric, partition)
        return rcluster
    else:
